...
import { MarketDto, TickerDto } from "@/app/types";
                            ...
                            <Card key={market.code} className="bg-white shadow-sm rounded-sm border-0">
                                <CardContent className="p-4">
                                    ...
                                    <div className="flex justify-between items-end mt-1">
                                        <p className="text-xl font-semibold">0{" "}<span className="text-xs">{market.code.split("-")[0]}</span></p>
                        ...
                        <Card key={market.code} className="bg-white shadow-sm rounded-sm border-0">
                            <CardContent className="p-4">
                                ...
                                <div className="flex justify-between items-end mt-1">
                                    <p className="text-xl font-semibold">
                                        {new Intl.NumberFormat(undefined, {
                                            minimumFractionDigits: ticker.tradePrice <= 1
                                                ? 8
                                                : ticker.tradePrice < 1000
                                                    ? 1
                                                    : 0,
                                            maximumFractionDigits: ticker.tradePrice <= 1
                                                ? 8
                                                : ticker.tradePrice < 1000
                                                    ? 1
                                                    : 0,
                                    ...
                                    <p className={ticker.signedChangeRate >= 0 ? "text-sm text-red-500" : "text-sm text-blue-500"}>
                                ...
                                <p className="text-xs text-gray-500 mt-2">
                                    거래량: {new Intl.NumberFormat().format(parseFloat(ticker.accTradeVolume?.toFixed(3) || '0'))}
                                </p>
                            </CardContent>
                        </Card>
                    </Link>
