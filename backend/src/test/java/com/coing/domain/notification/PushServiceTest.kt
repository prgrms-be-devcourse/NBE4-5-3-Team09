...
class PushServiceTest {
    ...
    fun updateSubscription_success() {
        val user = getTestUser()
        ...
        pushService.updateSubscription(userId, market.code, newRate, oldRate)

        verify(firebaseMessaging).unsubscribeFromTopic(tokens, "${market.code}-HIGH-3")
        verify(firebaseMessaging).unsubscribeFromTopic(tokens, "${market.code}-LOW-3")
        verify(firebaseMessaging).subscribeToTopic(tokens, "${market.code}-HIGH-5")
        verify(firebaseMessaging).subscribeToTopic(tokens, "${market.code}-LOW-5")
    ...
    fun getSubscribeInfo_success() {
        val user = getTestUser()
        val market = getTestMarket()
        val subscribe = Subscribe(user = user, market = market, oneMinuteRate = OneMinuteRate.THREE)
