...
class CandleServiceIntegrationTest {
    ...
    fun `실제 API 호출 - seconds 캔들 데이터 조회`() {
        ...
        val candles = candleService.getLatestCandles("KRW-BTC", EnumCandleType.seconds, null)
    ...
    fun `실제 API 호출 - minutes 캔들 데이터 조회`() {
        val candles = candleService.getLatestCandles("KRW-BTC", EnumCandleType.minutes, 1)
        assertNotNull(candles)
        println("실제 API로부터 ${candles.size}개의 캔들 데이터를 조회함.")
    }
}